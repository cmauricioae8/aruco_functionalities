...
public:
    VisualPoseEstimator() : Node("visual_pose_estimator") {
        ...
        // Setup Input
        if (camera_port_ >= 0) {
            RCLCPP_INFO(this->get_logger(), "Opening camera port %d at %.2f Hz", camera_port_, frequency);
...
private:
    ...
    void processImage(cv::Mat& image, rclcpp::Time stamp) {
        ...
        tf2::Quaternion avg_base_q(0, 0, 0, 0); 
        int valid_count = 0;
        ...
        for (size_t i = 0; i < ids.size(); ++i) {
            ...
            RCLCPP_INFO(this->get_logger(), "Yaw_c_m: %.3f", th_c_m);
            ...
            double z_b_m = tvecs[i][2]; // z offset

            double th_b_m_aux = th_c_m - M_PI_2; 
            double th_b_m = atan2(sin(th_b_m_aux),cos(th_b_m_aux)); // to get -M_PI < th < M_PI
            RCLCPP_INFO(this->get_logger(), "T_b_m: (x,y,z,th): (%.3f,%.3f,%.3f,%.3f)", x_b_m, y_b_m, z_b_m, th_b_m);
            ...
            double Z_w_b = z_b_m + marker_w.z;

            RCLCPP_INFO(this->get_logger(), "T_w_b: (x,y,th): (%.3f,%.3f,%.3f)", X_w_b, Y_w_b, th_w_b);
            
            // Transformation from world (odom) to marker (facing down)
            tf2::Quaternion avg_base_q;
            avg_base_q.setRPY(0,0,th_w_b);
            
            avg_base_pos[0] += X_w_b;
            avg_base_pos[1] += Y_w_b;
            avg_base_pos[2] += Z_w_b;
        ...
        if (valid_count > 0) {
            ...
            pose_msg.pose.position.z = avg_base_pos[2];
    ...
    std::string base_frame_, odom_frame_, camera_frame_;
